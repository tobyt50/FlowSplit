...
async function bootstrap() {
  const app = await NestFactory.create(AppModule, {
    bufferLogs: true,
    // Critical for Stripe Webhooks to verify signatures
    rawBody: true, 
  });
  ...
  const port = configService.get<string>('CARD_SERVICE_PORT') || 3110;
  await app.listen(port);

  pinoLogger.log(`🚀 Card Service running on: http://localhost:${port}`);
}
bootstrap();